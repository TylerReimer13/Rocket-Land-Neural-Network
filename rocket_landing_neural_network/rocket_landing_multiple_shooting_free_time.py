...
class TrajPlanner:
    ...
    def solver(self, x_dot):
        ...
        k1, k1_q = f(X, U)
        X = X + k1 * self.DT  # Integrated state ODE's
        Q = Q + k1_q * self.DT
    ...
    def __call__(self,  max_itr=5000, show_plot=False):
        xdot = self.dynamics()
        F = self.solver(xdot)
        ...
        solver_opts = {'ipopt': {'max_iter': max_itr}}
        ...
        # Plot the solution
        # Solver outputs are in a flattened list: [DT_opt (only one), states_opt_k1, controls_opt_k1, state_opt_k2...]
        self.DT_opt = w_opt[0]
        print('DT: ', self.DT_opt)

        self.x_opt = w_opt[1::self.NUM_STATES + self.NUM_INPUTS]
        self.y_opt = w_opt[2::self.NUM_STATES + self.NUM_INPUTS]
        self.thet_opt = w_opt[3::self.NUM_STATES + self.NUM_INPUTS]
        self.m_opt = w_opt[4::self.NUM_STATES + self.NUM_INPUTS]
        self.x_dot_opt = w_opt[5::self.NUM_STATES + self.NUM_INPUTS]
        self.y_dot_opt = w_opt[6::self.NUM_STATES + self.NUM_INPUTS]
        self.thet_dot_opt = w_opt[7::self.NUM_STATES + self.NUM_INPUTS]

        self.u1_opt = w_opt[8::self.NUM_STATES + self.NUM_INPUTS]
        self.u2_opt = w_opt[9::self.NUM_STATES + self.NUM_INPUTS]
    ...
    def plot(self):
        states_time = [n * self.DT_opt for n in range(self.N + 1)]
        controls_time = [n * self.DT_opt for n in range(self.N)]
...
if __name__ == "__main__":
    mass = 100.
    start = {'x': -45., 'y': 120., 'thet': 0., 'm': mass, 'x_dot': 0., 'y_dot': 0., 'thet_dot': 0.}
    end = {'x': 0., 'y': 0., 'thet': 0., 'x_dot': 0., 'y_dot': 0., 'thet_dot': 0.}

    misc = {'nu': 100}

    guidance = TrajPlanner(start, end, misc)
    status = guidance(show_plot=True)
